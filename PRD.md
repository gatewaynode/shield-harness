...
- **2026-04-27** — Pin bumped to lcs ≥ 0.5.3. Phase 2a probing revealed lcs 0.5.2 omitted `threat_scores` from the JSON on clean (`clean=true`) responses; lcs 0.5.3 ships the fix and now emits `threat_scores: {class_scores: {}, cumulative: 0}` for clean across all three engines. Phase 2a `runner::invoke` landed against 0.5.3: `ScanReport` parser models every 11.5 field as required (no `Option<>`); `scan()` pipes stdin, captures stdout/stderr/exit/wall-clock latency. UC-1..UC-6 unaffected; this is purely a contract pin bump and a Phase 2a code drop.
