...
| Subprocess-only forbids library introspection (§5 non-goal) | Per-rule attribution comes from `findings[].rule_name` and `findings[].engine`, populated directly by lcs ≥ 0.5.3 in every `ScanReport`. Log-scrape is no longer required for attribution. |
...
Required: lcs ≥ 0.5.3 (the 0.5.3 fix guarantees `threat_scores` is present on every `ScanReport`, including clean responses, so the harness models it as a required field with no `Option<>`).
...
- **2026-04-27** — Pin bumped to lcs ≥ 0.5.3 after Phase 2a probe surfaced an upstream gap: lcs 0.5.2 omitted `threat_scores` from the JSON when `clean=true`. Filed upstream; lcs 0.5.3 ships `threat_scores: {class_scores: {}, cumulative: 0}` for clean responses across all three engines (per-engine fingerprints unchanged). §12.1 pin line updated. Phase 2a `runner::invoke` lands: `ScanReport`/`Finding`/`ThreatScores` types in `runner::scan_report` (all required fields, no `Option<>`); `runner::lcs::binary` shared resolver (`introspect::binary` refactored onto it); `scan(sample_bytes, engine, lcs_path)` in `runner::invoke` pipes stdin, captures stdout/stderr/exit/wall-clock latency, returns `ScanOutcome` (or `ScanError::{LcsNotFound, StdinFailed, WaitFailed, Crashed, ParseFailed, UnexpectedExit}`); 39 tests pass including 7 live integration tests against lcs 0.5.3 (clean exit-0, threat exit-1 with prompt_injection, multi-finding syara, raw-stdout round-trip, latency capture, ParseFailed via /bin/echo stand-in).
